...
uab=subs(ua,[theta1,y],[theta1b,yb]);

simplify(uab-ua)


x = [alpha0; alpha1; sig; siga ];
...
obju = -(...
          bobs*log( (1/2)*(1+ erf((wobs-simplify(uab-ua))/(siga*sqrt(2))) ) )  ...
    + (1-bobs)*log( 1 -  (1/2)*(1+ erf((wobs-simplify(uab-ua))/(siga*sqrt(2))) ) ) ...
        );

jju=np*subs(jacobian(obju,x),0,es)
hhu=np*reshape(subs(hessian(obju,x),0,es),1,size(x,1)^2)

matlabFunction(obju,simplify(jju),simplify(hhu),'File','ghu2.m',...
                    'Outputs',{'val','g','h'})
...
matlabFunction(obj,simplify(jj),simplify(hh),'File','gh2.m',...
                    'Outputs',{'val','g','h'})
