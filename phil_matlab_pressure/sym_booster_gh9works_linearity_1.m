...
syms c   nc   np
syms EV  AV AVB  TV TVB DV UV  A1UV A2UV OV KV  AVN TVN gamma

syms JJ1 JJ2 JJ3 JJ4 JJ5 JJ6 JJ7 JJ8 JJ9 JJ10 JJ11 t f 
...
u = x + (1/alpha1).*(f*t - .5*( f*t - alpha0 )^2) - gamma.*t

% u = x + alpha1*sqrt(w)
lan  =  u  + l*BC

dt = simplify(diff(lan,t))
dx = diff(lan,x)
dl = diff(lan,l)

[ta,xa,la]  =  solve([dt,dx,dl],[t,x,l]);

ua = simplify(subs(u,[t,x],[ta,xa]));
