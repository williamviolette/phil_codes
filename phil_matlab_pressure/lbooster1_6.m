function  [h,um,bm] = lbooster1_6(input,Bobs,wobs,p,y,c,post,A,T,given)
...
%     ep = normrnd(0,sig,n,2);
    ...
    v1s = up1(  a0,alpha1,p,theta1.*(1-B) + post.*theta2  + post.*(1-B).*theta3,y)  + e(:,1);
    v2s = up1(  a0,alpha1,p,theta1.*B + post.*theta2  + post.*(B).*theta3,y - c) + e(:,2);
