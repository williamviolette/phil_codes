...
u = x + (1/alpha1).*(theta1*w1 - .5*( w1 - alpha0 )^2)  + (1/alpha1).*(.5*( w2 - alpha0 )^2)

% u = x + alpha1*sqrt(w)
lan  =  u  + l*BC

dw1 = simplify(diff(lan,w1))
dw2 = simplify(diff(lan,w2))
dx = diff(lan,x)
dl = diff(lan,l)

[wa1,wa2,xa,la]  =  solve([dw1,dw2,dx,dl],[w1,w2,x,l]);
 
ua11 = simplify(subs(u,[w1,w2,x],[wa1,wa2,xa]));
...
u = x + theta1*(1/alpha1).*(w - .5*( w - alpha0 )^2)
