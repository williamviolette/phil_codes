...
wa1 = subs(wa,alpha0,alpha0_1*A_1+alpha0_2*A_2+AC);

ua = simplify(subs(u,[w,x],[wa,xa]));
ua1= simplify(subs(u,[w,x],[wa1,xa]));
expand(ua)

uab1=subs(ua1,[theta1,y],[theta1b,yb]);

simplify(uab1-ua1)


x = [alpha0_1; alpha0_2; alpha1; sig ];
...
matlabFunction(obj,simplify(jj),simplify(hh),'File','gh3.m',...
                    'Outputs',{'val','g','h'})
