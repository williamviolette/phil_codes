...
uab = subs(ua,[theta],[T_1 + T_2*R + T_3*R]);
uan = subs(ua,[theta],[ T_2*R ]);


wab = subs(wa,[theta],[T_1 + T_2*R + T_3*R]);
wan = subs(wa,[theta],[ T_2*R ]);


wobs = ( wab*((uab-uan)-F)/(F1-F2) ) + ...
      ( wan*(1-(((uab-uan)-F)/(F1-F2))) );

simplify(wobs)

simplify(diff(wobs,R))


ua_post = subs(ua,theta,thetab);
...
matlabFunction(obju1 + obju2,jj,hh,'File','ghu2s1.m',...
                    'Outputs',{'val','g','h'},'Optimize',true) % 77 sec with full + siga  
toc
                
matlabFunction(simplify(wa,'IgnoreAnalyticConstraints',true,'Steps',steps),'File','wp2s1.m')
matlabFunction(simplify(ua,'IgnoreAnalyticConstraints',true,'Steps',steps),'File','up2s1.m')
