...
Aval = (alpha0_1*A_1+alpha0_2*A_2+AC);
...
j1 = subs(jacobian(obju1,x),[Eval,Uval,Aval,Tval,Dval],[EV,UV,AV,TV,DV]);

h1 = subs(hessian(obju1,x),[A1Uval,A2Uval],[A1UV,A2UV]);
h1 = subs(h1,[Eval,Uval,Aval,Tval,Dval],[EV,UV,AV,TV,DV]);
...
matlabFunction(obju1,jj,hh,'File','ghu5_1.m',...
...
matlabFunction(obju2,jj,hh,'File','ghu5_2.m',...
                    'Outputs',{'val','g','h'},'Optimize',true)
toc
...
matlabFunction(simplify(obj),simplify(jj),simplify(hh),'File','gh5.m',...
                    'Outputs',{'val','g','h'},'Optimize',true)
toc
