...
tic
jj=np*subs(j1s,0,es)
matlabFunction(obju1 + obju2 + obj,jj,'File','ghu8_vj.m',...
                    'Outputs',{'val','g'},'Optimize',true) % 77 sec with full + siga  
toc


x_alpha = [alpha0_1; alpha0_2;   alpha1; sig; siga ];

tic
h1t= hessian(obju1 + obju2  + obj,  x_alpha );
h1s = simplify(subs(h1t,sub_set,sub_var));
hh=np*reshape(subs(h1s,0,es),1,size(h1s,1)^2)
j1 = subs(jacobian(obju1 + obju2 + obj,x_alpha),sub_set,sub_var);
j1s = simplify(j1);
jj=np*subs(j1s,0,es)
matlabFunction(jj,hh,'File','ghu8_halpha.m',...
                    'Outputs',{'g','h'},'Optimize',true) % 77 sec with full + siga  
toc


x_theta = [theta0_1; theta0_2;   alpha1; sig; siga ];
tic
h1t= hessian(obju1 + obju2  + obj,  x_theta );
h1s = simplify(subs(h1t,sub_set,sub_var));
hh=np*reshape(subs(h1s,0,es),1,size(h1s,1)^2)
j1 = subs(jacobian(obju1 + obju2 + obj,x_theta),sub_set,sub_var);
j1s = simplify(j1);
jj=np*subs(j1s,0,es)
matlabFunction(jj,hh,'File','ghu8_htheta.m',...
                    'Outputs',{'g','h'},'Optimize',true) % 77 sec with full + siga  
toc

x_alpha_theta = [alpha0_1; alpha0_2; theta0_1; theta0_2 ];
tic
h1t= hessian(obju1 + obju2  + obj,  x_alpha_theta );
h1s = simplify(subs(h1t,sub_set,sub_var));
hh=np*reshape(subs(h1s,0,es),1,size(h1s,1)^2)
j1 = subs(jacobian(obju1 + obju2 + obj,x_alpha_theta),sub_set,sub_var);
j1s = simplify(j1);
jj=np*subs(j1s,0,es)
matlabFunction(jj,hh,'File','ghu8_halpha_theta.m',...
                    'Outputs',{'g','h'},'Optimize',true) % 77 sec with full + siga  
toc
