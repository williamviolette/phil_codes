...
* use "${temp}capex_dma_full.dta", clear
* 	g cper = cost/pipe_l
* 	hist cper if cper<40
* 	scatter cper  pipe_l  if cper<40
* 	scatter cper  pipe_l  if cper<40 & pipe_l<20
* 	reg cper pipe_l
* 	sum pipe_l
* 	sum cper
* * use  "${temp}zone_dma_link.dta",  clear


* file with mru dma zone ba :
...
gegen cper_m   = mean(cper),   by(zone_code)
gegen length_m = mean(length), by(zone_code)
gegen accts_m  = mean(accts),  by(zone_code)
gegen sho_m    = mean(SHO),    by(zone_code)
...
* gegen ztag=tag(zone_code) * 80 zones
* gegen zmax=max(post_treated), by(zone_code) * 54 zones have some treatment
* but: we're only focusing on 80 zones?
* gegen mtag=tag(mru) * 2976 mru
...
g zsamp = ppre>100 & ppre<. & ppost>100 & ppost<.
replace zone_code = . if zsamp==0
...
reghdfe amount ZM_*_post_treated [pweight=SHO], a(conacct date) cluster(mru)
mat def ae = e(b)
...
use "${temp}mru_dma_link.dta", clear
	...
	* 1.4 is mean SHO
	...
	merge m:1 dma using "${temp}zone_dma_link.dta", keep(3) nogen
	merge m:1 zone_code using "${temp}id_zone_set.dta", keep(1 3) nogen

	duplicates drop dma, force
	keep  dma  dma_pop zm zone_code

* gegen dtag=tag(dma)
* gegen dmtag=tag(dma zm)
* OUTPUT: DMA, zone, accts
...
* 	merge m:1 dma using "${temp}id_dma_set.dta", keep(1 3) nogen
* gegen dtag=tag(dma)
