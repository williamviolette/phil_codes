...
save "${temp}c10_`r'_hh_pop.dta", replace 
}


use "${temp}c10_1339_hh_pop.dta", clear
foreach r in 1375 13741 13742 13761 13762  {
	append using "${temp}c10_`r'_hh_pop.dta"
}
save "${temp}c10_hh_pop.dta", replace
...
foreach v in "R13_CITY OF LAS PIAS _PRV7601.DAT"  "R13_CALOOCAN CITY _PRV7501.DAT" "R13_CITY OF MAKATI _PRV7602.DAT" "R13_CITY OF MALABON_PRV7502.DAT" "R13_CITY OF MANDALUYONG _PRV7401.DAT" "R13_CITY OF MANILA_PRV39.DAT" "R13_CITY OF MARIKINA_PRV7402.DAT" "R13_CITY OF MUNTINLUPA_PRV7603.DAT" "R13_CITY OF NAVOTAS_PRV7503.DAT" "R13_CITY OF PARAAQUE_PRV7604.DAT" "R13_CITY OF PASIG _PRV7403.DAT" "R13_CITY OF SAN JUAN _PRV7405.DAT" "R13_CITY OF VALENZUELA _PRV7504.DAT" "R13_PASAY CITY _PRV7605.DAT" "R13_PATEROS_PRV7606.DAT" "R13_QUEZON CITY _PRV7404.DAT" "R13_TAGUIG CITY_PRV7607.DAT" {
	* local v "R13_CITY OF MANILA_PRV39.DAT"
	...
	g o=1
	gegen pop15=sum(o), by(bar)
	drop o
	duplicates drop bar, force
	save "${temp}c15_${j}_hh_pop.dta", replace
	global j = $j +1
}

use "${temp}c15_1_hh_pop.dta", clear
forvalues r=2/17 {
	append using "${temp}c15_`r'_hh_pop.dta"
}
save "${temp}c15_hh_pop.dta", replace
	...
	* year expand
	...
	use "${temp}brgy_link.dta", clear
		...
		ren prikeyc254 prikey
			...
			drop if tiny_overlap<=.01
			drop tiny_overlap
		merge m:1 mru using "${temp}mru_shh.dta", keep(3)
		*** share of barangay area that intersects with mru
			gegen int_per_bar = sum(int_area), by(bar)
		*** share of total barangay intersection attributed to each mru
			g mru_weight      = int_area/int_per_bar
		...
		*** share of mru that contributes to intersection area
			g mru_fraction    = int_area/mru_area
	save "${temp}mru_to_bar_shh.dta", replace
	...
	use "${data}paws/clean/full_sample_b_1.dta", clear
			...
			ren shh shh1
			gegen shh=mean(shh1), by(mru)
			keep shh mru
			duplicates drop mru, force

		merge 1:m mru using "${temp}mru_to_bar_shh.dta", keep(3) nogen

		foreach var of varlist shh {
			ren `var' `var'temp
...
odbc load, exec("SELECT * FROM meter_barangay_int")  dsn("phil") clear  
	keep prikey
	destring prikey, replace force
	g o=1
	gegen totmet=sum(o), by(prikey)
	drop o
	duplicates drop prikey, force
save "${temp}meter_pop.dta", replace
...
* use "${temp}c10_pop.dta", clear
	...
	merge m:1 prikey using "${temp}meter_pop.dta", keep(1 3) nogen
	merge m:1 bar using "${temp}c15_hh_pop.dta", keep(3) nogen
	merge m:1 bar using "${temp}c10_hh_pop.dta", keep(3) nogen
	merge m:1 bar using "${temp}bar_shh.dta", keep(1 3) nogen
...
g dens = pop15/bar_area


g atot=aressum*shh

g atot_pop15=atot/pop15
g atot_pop10=atot/pop10

sum atot_pop10 if year==2010, detail
sum atot_pop15 if year==2014, detail



g ares_pop15=aressum/pop15

g totmeter_pop15=totmet/pop15

sum ares_pop15 		if year==2014, detail 

reg ares_pop15 

sum totmeter_pop15 	if year==2014, detail
...
g share_ratio = pop15/aressum

* sum share_ratio, detail
* sum shh, detail

hist ares_pop15 if  ares_pop15<1.5





hist atot_per if atot_per<5
