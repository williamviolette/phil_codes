...
use "${pawsdata}full_sample.dta", clear
	...
	g date = ym(year,month)
		...
		** KEY SHARING CUTOFF HERE
		...
		keep conacct barangay_id wave date $paws_vars
		order conacct barangay_id wave date $paws_vars
		* house_1 house_2 age hhemp hhsize low_skill SHH SHO

odbc exec("DROP TABLE IF EXISTS paws_date;"), dsn("phil")
odbc insert, table("paws_date") dsn("phil") create
odbc exec("CREATE INDEX paws_conacct_date_ind ON paws_date (conacct);"), dsn("phil")
