...
use "${data}paws/clean/full_sample_b_1.dta", clear
	...
	keep if tn==tnm
	keep conacct SHH year single apartment
...
reg c i.pTc i.yearc i.ba if pTc<=11
...
keep conacct mru pi_res pr_res alpha0 alpha1 theta SHH single apartment popd
order conacct mru pi_res pr_res alpha0 alpha1 theta SHH single apartment popd
	duplicates drop conacct, force
	expand SHH
	sort conacct
