function [t,Q_obs,k_1,k_2,k_3,p_1,p_2,p_3,p_4,CONTROL]=...
    data_prep(est_version,sample,size_smp,mac,tag,tag_g,control_size,RLENGTH,BOOT)
...
%}
    if mac==1
        cd '/Users/williamviolette/Google Drive/jmp/sim_build'
    else
        cd 'C:\Users\wviolett\Google Drive\jmp\sim_build'
    end
...
if length(sample)>3
    if sample(4)==1
        t4      = csvread(strcat('pre_t_',tag_g,'.csv'),1,0);
        X4      = csvread(strcat('pre_',tag_g,'.csv'),1,0, [ 1 0 sum(t4) (4+control_size) ]);
            if isempty(BOOT)~=1
                g          = csvread(strcat('g_',tag_g,'.csv'),1,0)  ;   
                [~,~,gBindex,i_GB,t4] = boot_testing(g,t4);
                X4      = csvread(strcat('pre_',tag_g,'.csv'),1,0, [ 1 0 max(i_GB) (4+control_size) ]);
                X4=X4(i_GB,:); %%
                %{
                    if BOOT(1)>1
