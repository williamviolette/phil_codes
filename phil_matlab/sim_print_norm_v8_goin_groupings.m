...
if appending~=1
    fileID=fopen(strcat('eNORM',num2str(est_version),' ',est_title,'.txt'),'w');
    fprintf(fileID,'%s\r\n',est_title);
    fprintf(fileID,'%s\r\n',' ');
end
...
[~]=sim_build_normc_real_data_smm_v5_more_moments_groupings(print,tag,mac,size_smp,...
 ...
 %[~]=sim_build_normc_real_data_smm_v4(print,tag,mac,size_smp,...
 %                   fileID,est_version,controls,control_max,ph_controls,given,...
 %                   [ 200 200 40 10 ]  ,sto,reps,...
 %                   sort_condition,split_F_option,1,[4 2 50],...
 %                   1,TUNE,BOOT,boot_max,boot_estimates,alt_sample,ESTIMATION_OPTION,COST_INPUTS,C_FEE_DISCOUNT,income_percentile ); % set real data
                %} 
    %{a
end
    %}    
...
%{
        ...
        [~]=table_step_3_estimates...
            (est_version,tag,boot_max,mac,control_max,size_smp,alt_sample);
